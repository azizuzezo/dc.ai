...
TTS uses the viewer's browser's built-in speech synthesis (instant, no
network round trip), so reliability depends on that browser/OS having voices
installed — test it once in the actual OBS/TikTok Live Studio browser source
before going live.
...
in/out, optionally beeps, and reads the message aloud via the browser's
Web Speech API.
